...
# Explicit overrides for summaries not covered by Showdown's reusable shortDesc
# translations. Keys are stable Showdown/PokéAPI move slugs so later wording
# changes in the English source cannot silently reintroduce fallbacks.
GERMAN_MOVE_SUMMARY_OVERRIDES: dict[str, str] = {
    "belch": (
        "Kann nur eingesetzt werden, nachdem der Anwender im Kampf eine "
        "Beere gegessen hat."
    ),
    "freezedry": (
        "10% Chance, das Ziel einzufrieren. Sehr effektiv gegen "
        "Wasser-Pokémon."
    ),
    "moonblast": (
        "10% Chance, den Sp.-Ang. des Ziels um 1 Stufe zu senken."
    ),
    "toxicthread": (
        "Vergiftet das Ziel und senkt seine Initiative um 2 Stufen."
    ),
    "stuffcheeks": (
        "Der Anwender muss eine Beere tragen. Er isst sie und erhöht seine "
        "Verteidigung um 2 Stufen."
    ),
    "direclaw": (
        "30% Chance, das Ziel einzuschläfern, zu vergiften oder zu "
        "paralysieren."
    ),
    "saltcure": (
        "Verursacht jede Runde Schaden in Höhe von 1/8 der maximalen KP des "
        "Ziels; bei Wasser- und Stahl-Pokémon 1/4."
    ),
    "makeitrain": (
        "Senkt den Sp.-Ang. des Anwenders um 2 Stufen. Trifft beide Gegner."
    ),
    "ragefist": (
        "Die Stärke steigt für jeden Treffer, den der Anwender erlitten hat, "
        "um 50; maximal nach 6 Treffern."
    ),
}


# Some Champions balance changes are present in Showdown's structured fields
# but not in its inherited prose, so the affected texts are normalized here.
MOVE_TEXT_OVERRIDES: dict[str, dict[str, str]] = {
    "moonblast": {
        "summary_en": "10% chance to lower the target's Sp. Atk by 1.",
        "description_en": (
            "Has a 10% chance to lower the target's Special Attack by 1 stage."
        ),
    },
    "toxicthread": {
        "summary_en": "Lowers the target's Speed by 2 and poisons it.",
        "description_en": (
            "Lowers the target's Speed by 2 stages and poisons it."
        ),
    },
    "direclaw": {
        "summary_en": "30% chance to sleep, poison, or paralyze target.",
        "description_en": (
            "Has a 30% chance to cause the target to either fall asleep, "
            "become poisoned, or become paralyzed."
        ),
    },
    "makeitrain": {
        "summary_en": "Lowers the user's Sp. Atk by 2. Hits foe(s).",
        "description_en": "Lowers the user's Special Attack by 2 stages.",
    },
}
...
def apply_move_text_overrides(move: dict[str, Any]) -> None:
    """Apply known Champions text corrections before localization."""
    overrides = MOVE_TEXT_OVERRIDES.get(str(move.get("api_name") or ""))
    effects = move.get("effects")
    if overrides is None or not isinstance(effects, dict):
        return
...
def add_german_summary(move: dict[str, Any]) -> bool:
    ...
    summary_de = GERMAN_MOVE_SUMMARY_OVERRIDES.get(
...
def import_moves(
    ...
    for position, move in enumerate(moves, start=1):
        ...
        apply_move_text_overrides(move)
